...
idx2=0+round(p.wsize/2/p.hop);%按块计算
%-----LUWEI ADDED START--------
...
% 	if size(xx,1) == 1; xx=xx'; end
	...
	[prd,ap0,ap,pwr,good_flag]=yin_helper(xx,p); %for each chunk, perform yin,一个块一个块的计算
	n=size(prd ,2);
	if (~n) break; end
	idx=idx+n*p.hop;
...
dd=zeros(floor((m-mxlg-p.hop)/p.hop),mxlg);%36*443%m长度为10000的buffer这一样才有多个
...
dd=dd';
[dd,ddx]=minparabolic(dd); 	% parabolic interpolation near local min
