...
%  voicingProb denotes the initial voicing probability
    ...
    methods
        ...
        function obj = BayesianfastF0NLS(N, L, pitchBounds, A_var,voicingProb,c,varargin)
            switch nargin
                case 0
                     N=0.03*44100;%16000->44100,no low-pass, may be slower
                     L=10;
                     pitchBounds=[50/44100,400/44100];
                     A_var=[2/44100];
                     voicingProb=0.7;
                     c=[.7 .3;.4,.6];
                case 1
...
%                   cod = costs*(1/(x'*x));
              ...
              %% added/modified by Liming Shi (implements the hidden markov chain)
                if isnan(obj.scaled_alpha_buffer)                
                    ...
                    estimatedPitch=maxparabolic(scaled_alpha(:,estimatedOrder),obj.fullPitchGrid,pitchIndex(1));%Dequantification using parabolic interpolation;
