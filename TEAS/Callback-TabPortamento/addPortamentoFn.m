...
%ADDPORTAMENTOFN Add portamento
   ...
   if validNewPortamento == 1
        if isfield(data,'portamentos') == 1
            ...
            data.portamentos=[data.portamentos;newPortamentoStart,newPortamentoEnd,newPortamentoEnd-newPortamentoStart,1];
            [~,index] = sort(data.portamentos(:,1));
            data.portamentos=data.portamentos(index,:);
            indexNewPortamento = find(index(:,1) == size(index,1));
            data.PortamentoType.Value=1;
            ...
            if isfield(data,'pitchVFree')
                ...
                msgbox('Press button ''Vibrato-free Pitch'' first.');
                return
            end
