...
%APPLYLOGISTICTOALLFN apply Logistic model to all portamentos
    h = waitbar(0,'Poramento parameter fitting using logistic models...');    
    global data;
...
%         InflectionPointTime = log(fittedDataLogistic6(i,1)./fittedDataLogistic6(i,2))./(fittedDataLogistic6(i,3))+fittedDataLogistic6(i,5);
        ...
        %-----------------------------------------     
        waitbar(i/length(portamentosName),h,sprintf('%d%% Poramento parameter fitting using logistic models...',round(100*i/length(portamentosName))));
