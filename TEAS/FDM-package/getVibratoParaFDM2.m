function [ vibratoPara ] = getVibratoParaFDM2( vibratos,time,FDMoutput,thres)
...
%   @vibratoPara: the matrix storing vibrato para. each row is one vibrato: [rate,extent]
    ...
    for i = 1:size(vibratos,1)
        ...
        passageFDMoutput(passageFDMoutput(:,1)>thres(1),1)=nan;
        passageFDMoutput(passageFDMoutput(:,1)>thres(1),2)=nan;
        passageFDMoutput(passageFDMoutput(:,2)>thres(2),1)=nan;
        passageFDMoutput(passageFDMoutput(:,2)>thres(2),2)=nan;
