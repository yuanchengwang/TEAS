    ...
    for i = 1:frameNum
        ...
        if(isempty(peakAmplitude) == 1)
            ...
            %look at both the vibrato rate limit and vibrato extent limit
                if(peakFrequency(i)<=vibratoRateLimit(2)&&peakFrequency(i)>=vibratoRateLimit(1) && (peakAmplitude(i)>=vibratoAmplitudeLimit(1))&& (peakAmplitude(i)<=vibratoAmplitudeLimit(2)))
                      vibratoIndicate(i) = indicateSign;
                end
        end
    end
