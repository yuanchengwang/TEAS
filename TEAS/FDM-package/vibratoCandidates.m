...
%   and ending time mark.
    ...
    if isempty(vibratoIndicateMark) == 1%the old code doesn't work for a single vibrato
        if prod(vibratoIndicateIndexDiff==1)%A single vibrato
            vibratoCandidateVector= [timeF(vibratoIndicateIndex(1)),timeF(vibratoIndicateIndex(end))];
        else %No vibrato
